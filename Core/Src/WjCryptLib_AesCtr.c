 ...
 * @param IV
 */
static void IncreaseIV(uint8_t* IV)
{
...
{
	//Kiet's implementation
	uint8_t current_cipher_key_stream[AES_BLOCK_SIZE];
	uint32_t total_encrypted_bytes = 0;
	while (1)
	{
		AesEncrypt(&Context->Aes, Context->IV, current_cipher_key_stream);
		XorBuffers(	current_cipher_key_stream,
								InBuffer + total_encrypted_bytes,
								OutBuffer + total_encrypted_bytes,
								MIN(Size - total_encrypted_bytes, AES_BLOCK_SIZE));
		total_encrypted_bytes += MIN(Size - total_encrypted_bytes, AES_BLOCK_SIZE);
		if (total_encrypted_bytes == Size)
		{
			//Finish encrypt
			break;
		}
		//Since this is AES CTR, increase next IV by 1
		IncreaseIV(Context->IV);
	}


	//Done Kiet's Implementation
